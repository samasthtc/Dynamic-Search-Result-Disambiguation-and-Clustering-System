# Add this at the top of clustering_algorithms.py after the other imports
...
try:
    ...
    print("   The system will use other clustering algorithms instead.")
...
class ClusteringManager:
    ...
    # Also update the supported_algorithms dictionary in __init__ to conditionally include hdbscan:
    ...
    def __init__(self):
        ...
        # Only add HDBSCAN if available
        if HDBSCAN_AVAILABLE:
            self.supported_algorithms["hdbscan"] = self._hdbscan_clustering
        ...
        algorithm_count = len(self.supported_algorithms) - 1  # Exclude adaptive
        self.algorithm_performance = {
            alg: [] for alg in self.supported_algorithms.keys() if alg != "adaptive"
        }

        logger.info(f"Clustering Manager initialized with {algorithm_count} algorithms")
        if not HDBSCAN_AVAILABLE:
            logger.info("Note: HDBSCAN algorithm not available")
    ...
    def cluster(
        ...
        **kwargs,
        ...
        labels = cluster_func(
            embeddings_scaled, num_clusters, min_cluster_size, **kwargs
        )
        ...
        logger.info(
            f"Clustering completed: {len(set(labels))} clusters, "
    ...
    def _kmeans_clustering(
        self, embeddings: np.ndarray, num_clusters: int, min_cluster_size: int, **kwargs
    ) -> np.ndarray:
        """
    ...
    def _optimize_kmeans_clusters(
        self, embeddings: np.ndarray, max_clusters: int
    ) -> int:
        """
    ...
    # Then replace the _hdbscan_clustering method with this version:
    ...
    def _hdbscan_clustering(
        ...
        if not HDBSCAN_AVAILABLE:
            ...
            return self._dbscan_clustering(
                embeddings, num_clusters, min_cluster_size, **kwargs
            )
    ...
    def _optimize_dbscan_epsilon(
        self, embeddings: np.ndarray, min_samples: int
    ) -> float:
        """
    ...
    def _bertopic_clustering(
        ...
        gmm = GaussianMixture(
            n_components=num_clusters, covariance_type="full", random_state=42
        )
    ...
    def _gaussian_mixture_clustering(
        self, embeddings: np.ndarray, num_clusters: int, min_cluster_size: int, **kwargs
    ) -> np.ndarray:
        """
    ...
    def _optimize_gmm_components(
        self, embeddings: np.ndarray, max_components: int
    ) -> int:
        """
    ...
    def _get_best_performing_algorithm(self) -> str:
        ...
        if not avg_performances or all(
            score == 0 for score in avg_performances.values()
        ):
    ...
    def _merge_small_clusters(
        self, labels: np.ndarray, min_cluster_size: int
    ) -> np.ndarray:
        """
    ...
    def _evaluate_clustering_quality(
        ...
        try:
            ...
            quality_score = (
                0.5 * silhouette_avg + 0.3 * ch_normalized + 0.2 * balance_score
            )
        ...
        for algorithm, scores in self.algorithm_performance.items():
            if scores:
                performance_stats[algorithm] = {
                    ...
                    "recent_score": (
                        np.mean(scores[-3:]) if len(scores) >= 3 else np.mean(scores)
                    ),
    ...
    def recommend_algorithm(self, data_characteristics: Dict[str, Any]) -> str:
        ...
        best_algorithm = max(
            performance_stats.items(), key=lambda x: x[1]["recent_score"]
        )[0]

        return best_algorithm

    def cluster_with_uncertainty(
        self, embeddings: np.ndarray, algorithm: str = "gaussian_mixture", **kwargs
        ...
        if algorithm == "gaussian_mixture":
            gmm = GaussianMixture(
                n_components=kwargs.get("num_clusters", 4), random_state=42
            )
    ...
    def _estimate_assignment_uncertainty(
        self, embeddings: np.ndarray, labels: np.ndarray
    ) -> np.ndarray:
        """
        ...
        for i, (embedding, label) in enumerate(zip(embeddings, labels)):
            ...
            if other_distances:
                ...
                uncertainty = 1.0 - (
                    dist_to_nearest_other / (dist_to_assigned + dist_to_nearest_other)
                )
            else:
                uncertainty = 0.0
    ...
    def interactive_clustering(
        self, embeddings: np.ndarray, user_constraints: Dict[str, Any]
    ) -> np.ndarray:
        """
        Perform clustering with user-provided constraints.
        """
        # Extract constraints
        must_link = user_constraints.get(
            "must_link", []
        )  # Pairs that must be in same cluster
        cannot_link = user_constraints.get(
            "cannot_link", []
        )  # Pairs that cannot be in same cluster
    ...
    def _apply_must_link_constraints(
        self,
        embeddings: np.ndarray,
        labels: np.ndarray,
    ...
    def _apply_cannot_link_constraints(
        self,
        embeddings: np.ndarray,
        labels: np.ndarray,
    ...
    def _apply_seed_constraints(
        self, labels: np.ndarray, seed_clusters: Dict[int, int]
    ) -> np.ndarray:
        """
    ...
    def _consensus_clustering(self, all_labels: List[np.ndarray]) -> np.ndarray:
        ...
        clustering = AgglomerativeClustering(
            n_clusters=consensus_k, metric="precomputed", linkage="average"
        )
